...
def hand_level(h:str) -> int:
    ...
    sh = h.replace("J", "")
    # 5 of a kind
    if wc == 5 or sh.count(sh[0]) == len(sh):
        return 7
    # 4 of a kind
    if sh.count(sh[0]) + wc == 4 or sh.count(sh[1]) + wc == 4:
        return 6
    # Full house / 3 of a kind
    if sh.count(sh[0]) + wc == 3 or sh.count(sh[1]) + wc == 3 or sh.count(sh[2]) + wc == 3:
        if sh.count(sh[0]) + wc == 3:
            sh = sh.replace(sh[0], "")
        elif sh.count(sh[1]) + wc == 3:
            sh = sh.replace(sh[1], "")
        elif sh.count(sh[2]) + wc == 3:
            sh = sh.replace(sh[2], "")
        if sh[0] == sh[1]:
            return 5
        else:
            return 4
    # Two Pair / One Pair
    if sh.count(sh[0]) + wc == 2 or sh.count(sh[1]) + wc == 2 or sh.count(sh[2]) + wc == 2 or sh.count(sh[3]) + wc == 2:
        if sh.count(sh[0]) + wc == 2:
            sh = sh.replace(sh[0], "")
        elif sh.count(sh[1]) + wc == 2:
            sh = sh.replace(sh[1], "")
        elif sh.count(sh[2]) + wc == 2:
            sh = sh.replace(sh[2], "")
        elif sh.count(sh[3]) + wc == 2:
            sh = sh.replace(sh[3], "")
